...
/**
 * SPEC-011 AR1–AR3 / AS1–AS3 / AT1–AT2 / AU1–AU2 / AV1 / R11-06–R16 — Composer shell,
 ...
 * Scan Now → SpiderfeetApi execute → four forms.
 */
(function ($, Composer, Widgets, document, window) {
   ...
   * On a completed four-form run, mark hasRun so AU2 keeps Scan Now disabled (AV2 replay later).
  ...
  /**
   * Programmatic Scan Now (console / verification). Respects current executeContext.
   ...
   * Wires executeContext for Scan Now → SpiderfeetApi (R11-16 / AV1).
  ...
  Composer.mountCliScanApp = function (opts) {
    ...
    // Composer has no run store yet (AV2); default unset gating until a host passes hasRun.
